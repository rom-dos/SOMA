...
/* -- sequence -- */
describe('sequence', () => {
  test.skip('returns a string (from 3 keys input)', () => {
    const test = sequence('c d e', 'major', 4, 'fold')
    expect(typeof test).toEqual('string')
  })

  test.skip('returns a string (from all major keys input)', () => {
    const seq = ['c', 'db', 'd', 'eb', 'e', 'f', 'gb', 'g', 'ab', 'a', 'bb', 'b']
    const test = sequence(seq, 'major', 4, 'fold')
    expect(typeof test).toEqual('string')
  })

  test.skip('returns a string (from all minor keys input)', () => {
    const seq = ['c', 'db', 'd', 'eb', 'e', 'f', 'gb', 'g', 'ab', 'a', 'bb', 'b']
    const test = sequence(seq, 'minor', 4, 'fold')
    expect(typeof test).toEqual('string')
  })

  test.skip('returns a sequence of 96 notes (all keys)', () => {
    const seq = ['c', 'db', 'd', 'eb', 'e', 'f', 'gb', 'g', 'ab', 'a', 'bb', 'b']
    const test = sequence(seq, 'major', 4, 'fold')
      .split(' ')
  ...
  test.skip('returns a sequnece of 24 notes (3 keys)', () => {
  ...
  test.skip('returns a sequence of 16 notes (2 keys)', () => {
